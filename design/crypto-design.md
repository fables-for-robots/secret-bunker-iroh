...
the first service admin is established at bootstrap.
...
- Server endpoint secret key (mounted from k8s Secret, generated by
  `keygen`).
- Operational age private key (mounted from k8s Secret, generated by
  `keygen`).
...
operational age private key. To protect them:
